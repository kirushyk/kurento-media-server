...
public:
  ...
  RTCRTPStreamStats (const std::string &id, std::shared_ptr<StatsType> type, double timestamp, const std::string &ssrc, const std::string &associateStatsId, bool isRemote, const std::string &mediaTrackId, const std::string &transportId, const std::string &codecId, int64_t firCount, int64_t pliCount, int64_t nackCount, int64_t sliCount, int64_t remb, int64_t packetsLost, double fractionLost) : RTCStats (id, type, timestamp) {
    this->ssrc = ssrc;
