...
public:
  ...
  RTCDataChannelStats (const std::string &id, std::shared_ptr<StatsType> type, double timestamp, const std::string &label, const std::string &protocol, int64_t datachannelid, std::shared_ptr<RTCDataChannelState> state, int64_t messagesSent, int64_t bytesSent, int64_t messagesReceived, int64_t bytesReceived) : RTCStats (id, type, timestamp) {
    this->label = label;
