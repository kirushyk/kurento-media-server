...
public:
  ...
  EndpointStats (const std::string &id, std::shared_ptr<StatsType> type, double timestamp, double inputAudioLatency, double inputVideoLatency, const std::vector<std::shared_ptr<MediaLatencyStat>> &inputLatency, double audioE2ELatency, double videoE2ELatency, const std::vector<std::shared_ptr<MediaLatencyStat>> &E2ELatency) : ElementStats (id, type, timestamp, inputAudioLatency, inputVideoLatency, inputLatency) {
